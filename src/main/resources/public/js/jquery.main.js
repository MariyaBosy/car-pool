jQuery(document)
		.ready(
				function($) {
					...
					/*
					 * fix vertical when not overflow call fullscreenFix() if
					 * .fullscreen content changes
					 */
					function fullscreenFix() {
						var h = $('.main-baner').height();
						// set .fullscreen height
						$(".second-parallax-content").each(
								function(i) {
					...
					function backgroundResize() {
						...
						$(".background").each(
								function(i) {
									...
									var diff = parseFloat(path
											.attr("data-diff"));
									diff = diff ? diff : 0;
									// remaining height to have fullscreen image
									// only on parallax
									var remainingH = 0;
									if (path.hasClass("parallax")
											&& !$("html").hasClass("touch")) {
										var maxH = contH > windowH ? contH
												: windowH;
									...
									path.css("background-size", imgW + "px "
											+ imgH + "px");
								});
					}
								...
								.each(
										function(i) {
											...
											if (bottomWindow > top
												...
												var imgH = path
														.data("resized-imgH");
												// min when image touch top of
												// window
												var min = 0;
												// max when image touch bottom
												// of window
												...
												// value with linear
												// interpolation
												var value = min + (max - min)
														* (currentWindow - top)
														/ (bottom - top);
												// set background-position
												var orizontalPosition = path
														.attr("data-oriz-pos");
